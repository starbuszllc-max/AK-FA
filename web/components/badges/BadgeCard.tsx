...
export function BadgeCard({ name, description, icon, earnedAt, locked }: BadgeCardProps) {
  ...
  return (
    <div className={`relative flex flex-col items-center p-4 rounded-lg border ${locked ? 'bg-gray-50 border-gray-200 opacity-60' : 'bg-gradient-to-br from-green-50 to-yellow-50 border-green-200'}`}>
      {locked && (
        <div className="absolute top-2 right-2">
          <Lock className="w-4 h-4 text-gray-400" />
        </div>
      )}
      <div className={`w-12 h-12 rounded-full flex items-center justify-center ${locked ? 'bg-gray-200' : 'bg-gradient-to-br from-green-400 to-yellow-500'}`}>
